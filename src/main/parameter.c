...
int read_param(param_t *param, FILE *fp, FILE *fp_log){
  ...
  /* [EVTol] */
  flexi_request_key("Tol",0,"%lg %lg", &param->etol,&param->vtol);
  /* default */
  param->etol = 1.e-8;   
  param->vtol = 1.e-6;   
  param->maxit = 100;      
  ...
  param->b = .013;
  param->ngrid = 1201;
  param->r = (double *)malloc(param->ngrid*sizeof(double));
  ...
  param->ngrid2 = 2201;
  param->r2 = (double *)malloc(param->ngrid2*sizeof(double));
  param->rab = (double *)malloc(param->ngrid2*sizeof(double));
  ...
  */
  ...
  /* if (streq(param->reltype,"nrl")) { */
    param->r[0] = param->a * pow(param->z, -1./3.);
    /*  } else {
    grid_.r[0] = param->a / param->z;
    }*/
  for (i=1; i<NPDM; i++) {
    param->r[i]=param->r[0] * exp(param->b * (i));
    if (param->r[i]>120.0) break;
    param->np++;
  }
  if (param->np > param->ngrid) {
    printf("check grid; # points needed %d > max np %d \n",param->np,param->ngrid);
    return 1;
  }
  if (param->r[param->np]<30.0) {
    printf("check grid; grid extends to less than 30 Bohr \n");
    return 1;
  }

  param->ngrid=param->np;
  ...
  param->np2=0;
  for (i=0; i<NPDM; i++){
    param->r2[i]=param->a2 * (exp(param->b2*(i))-1.0);
    param->rab[i] =(param->r2[i]+param->a2)*param->b2;
    if (param->r2[i]>80.0) break;
    param->np2++;
  }
...
/* Finally! Write the log file header */
  ...
  fprintf(fp_log," a_grid=%-11.2e b_grid=%-11.2e # points=%d \n r(1)=%-11.2e   r(np)=%-11.2e \n",
	  param->a,param->b,param->np,param->r[0],param->r[param->np-1]);
  /* }
